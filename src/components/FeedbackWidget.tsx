  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch("/api/feedback", {
        ...
        body: JSON.stringify({ type, message, repo: "ai-interview" }),
      });
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-72 bg-zinc-900 border border-white/10 rounded-xl p-4 space-y-3">
      ...
      <textarea
        ...
        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/40 focus:outline-none focus:ring-1 focus:ring-violet-400/50 resize-none"
      />
      <button
        onClick={handleSubmit}
        disabled={sending || !message.trim()}
        className="w-full bg-violet-500 text-white text-sm font-medium py-2 rounded-lg hover:bg-violet-600 transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {sending ? "送信中..." : "送信"}
      </button>
    </div>
